...
function renderBackupsList(sets) {
  ...
  const rows = sets.map(s => {
    ...
    // Boutons (pas des <a href>) : un lien classique ne peut pas fonctionner dès que le
    // frontend et le backend sont sur des origines différentes (voir
    // api-base.js::CROSS_ORIGIN) — il pointerait vers la mauvaise origine, et une simple
    // navigation ne peut de toute façon pas porter l'en-tête Authorization. Voir
    // downloadBackup() dans backups.js pour le téléchargement via fetch().
    const downloads = s.files.map(f => `<button class="btn small" data-download-backup="${f.name}">${f.kind}</button>`).join(' ');
    return `
    <tr>
...
function wireBackupsManager() {
  ...
  document.querySelectorAll('[data-download-backup]').forEach(dlBtn => {
    dlBtn.addEventListener('click', async () => {
      dlBtn.disabled = true;
      try {
        await downloadBackup(dlBtn.dataset.downloadBackup);
      } catch (e) {

...
// ================= RANG COMPÉTITIF (MMR façon Elo + paliers) =================
// EVA n'expose aucune notion de MMR/rang — c'est une statistique entièrement inventée et
...
// l'identifiant nu `rank` : uniquement `mmr`/`tier`. Le libellé UI "Rang" reste tel que
...
// convention que la plupart des jeux à ladder), chaque division large de DIVISION_WIDTH MMR.
...
const TIER_BASE = 100; // début de Bronze III, aligné avec MMR_FLOOR
...
// une fois au chargement du module, pas recalculée à chaque appel de mmrToTier(). Bronze III
// est ouverte vers le bas et Légende I ouverte vers le haut (min/maxExclusive = ±Infinity) —
// sécurité défensive pour mmrToTier(), même si MMR_FLOOR empêche déjà tout MMR réel de sortir
// de cette plage par le bas.
//
// Intervalles semi-ouverts [min, maxExclusive) — PAS [min, max] avec des bornes entières :
// le MMR est une valeur flottante (les deltas Elo ne tombent presque jamais sur un entier
...
// l'ancien repli retombait sur Légende I (bug réel constaté : un joueur à ~999 de MMR après
// seulement 4 parties affiché en Légende I, alors que BASE_MMR=1000 — début de Platine III —
...
// toujours un palier pour un MMR fini valide ; seul un mmr non-fini (NaN — ne devrait jamais
// arriver, voir computeMmrHistory) retombe sur le palier le plus bas plutôt que le plus haut,
...
export function mmrToTier(mmr) {
  const tier = TIERS.find(t => mmr >= t.min && mmr < t.maxExclusive) || TIERS[0];
  const progressPct = Math.max(0, Math.min(100, Math.round(((mmr - tier.naturalMin) / DIVISION_WIDTH) * 100)));
  return {
...
// par createdAt croissant, condition de correction du rejeu chronologique) et renvoie le MMR
...
// dans les deux Maps (pas de BASE_MMR par défaut juste pour exister) — laisse l'appelant
...
//
// Pour chaque partie : les deux équipes sont comparées façon Elo standard sur leur MMR moyen
// courant (BASE_MMR par défaut pour un joueur jamais vu). Le résultat réel de chaque joueur
...
// dans une défaite ferait perdre PLUS de MMR au lieu de moins).
export function computeMmrHistory(games, opts) {
  opts = opts || {};
  ...
  const baseMmr = opts.baseMmr != null ? opts.baseMmr : BASE_MMR;
  const mmrFloor = opts.mmrFloor != null ? opts.mmrFloor : MMR_FLOOR;
  ...
  const mmrByUid = new Map();
  const historyByUid = new Map();
  const mmrOf = uid => (mmrByUid.has(uid) ? mmrByUid.get(uid) : baseMmr);

  sorted.forEach(g => {
    const { teamA, teamB } = deriveTeams(g);
    const ratings = computeMatchRatings(g);

    const teamAAvg = teamA.reduce((sum, p) => sum + mmrOf(canonicalUid(p.userId)), 0) / teamA.length;
    const teamBAvg = teamB.reduce((sum, p) => sum + mmrOf(canonicalUid(p.userId)), 0) / teamB.length;
    const expectedA = 1 / (1 + Math.pow(10, (teamBAvg - teamAAvg) / 400));

    const applyTeam = (team, expected) => {
      team.forEach(p => {
        const uid = canonicalUid(p.userId);
        const oldMmr = mmrOf(uid);
        ...
        const newMmr = Math.max(mmrFloor, oldMmr + delta);

        mmrByUid.set(uid, newMmr);
        if (!historyByUid.has(uid)) historyByUid.set(uid, []);
        historyByUid.get(uid).push({
          gameId: g.id, createdAt: g.createdAt,
          mmrBefore: oldMmr, mmrAfter: newMmr, delta, outcomeDelta, perfBonus,
  ...
  return { mmrByUid, historyByUid, gamesUsed: sorted.length };
}

// ================= PORTÉE =================
// Portée dédiée au MMR — DÉLIBÉRÉMENT différente du contrat uniforme "période + saison +
...
// l'utilisateur) -> MMR rejoué depuis une base neutre, restreint aux parties de CETTE saison
...
// la portée du MMR. Les exclusions carte/mode (qualité de données, pas un filtre de période)
// s'appliquent, elles, toujours.
export function gamesForMmrScope() {
